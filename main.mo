  ...
  public shared (msg) func burn(id : Principal) : async Text {
    ...
    switch (nftOpt) {
      ...
      case (?nft) {
        ...
        let updatedList = List.filter(ownedNFTs, func(x : Principal) : Bool { x != id });

        mapOfOwners.put(owner, updatedList);

        // Remove from listings and NFTs
        mapOfListings.delete(id);
        mapOfNFTs.delete(id);
